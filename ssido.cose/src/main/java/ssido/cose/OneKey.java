...
public final class OneKey {
    ...
    public OneKey(PublicKey pubKey, PrivateKey privKey) throws CoseException {
        ...
        if (pubKey != null) {
            List<ASN1.TagValue> spki = ASN1.DecodeSubjectPublicKeyInfo(pubKey.getEncoded());
            List<ASN1.TagValue> alg = spki.get(0).list;
        ...
        if (privKey != null) {
            List<ASN1.TagValue> pkl = ASN1.DecodePKCS8(privKey.getEncoded());
            if (pkl.get(0).tag != 2) {
                throw new CoseException("Invalid PKCS8 structure");
            }
            List<ASN1.TagValue> alg = pkl.get(1).list;
            if (Arrays.equals(alg.get(0).value, ASN1.oid_ecPublicKey)) {
                byte[] oid = (byte[]) alg.get(1).value;
                if (oid == null) {
                    throw new CoseException("Invalid PKCS8 structure");
    ...
    /**
     * Compares the key's assigned algorithm with the provided value, indicating
     * if the values are the same.
     *
     * @param algorithmId the algorithm to compare or {@code null} to check for
     * no assignment.
     * @return {@code true} if the current key has the provided algorithm
     * assigned, or {@code false} otherwise
     */
    ...
    /**
     * Compares the key's assigned key type with the provided value, indicating
     * if the values are the same.
     *
     * @param keyTypeObj the key type to compare or {@code null} to check for no
     * assignment.
     * @return {@code true} if the current key has the provided identifier
     * assigned, or {@code false} otherwise
     */
     ...
     * indicating if the provided value was found in the key operation values
     * assigned to the key.
     *
     * @param that the integer operation value to attempt to find in the values
     * provided by the key or {@code null} to check for no assignment.
     * @return {@code true} if the current key has the provided value assigned,
 ...
 /*        
     ...
     * arbitrary data with a key and retrieve it later.
     *
     * @return
     */
